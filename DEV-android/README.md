# DEV — Voice-Controlled Android Assistant (MVP)
...
## Why I can't hand you a finished `DEV.apk` directly
...
2. A GitHub Actions workflow (`.github/workflows/build-apk.yml`) that builds and signs a real
   `DEV.apk` automatically using GitHub's own Android build environment — this is the closest
...
5. Download the signed `DEV.apk` from the workflow run's **Artifacts** section and install it
...
1. Install `DEV.apk`. If Android blocks it, enable "Install unknown apps" for the source
   (browser/file manager) you used, then retry.
2. Open DEV. On the **DEV** tab, grant microphone access when prompted.
3. Tap **Enable Accessibility Service** → find "DEV" in the list → turn it on. This is what
   lets DEV read screen content and tap/type/scroll/go-back in other apps.
4. Go to the **Settings** tab and paste in your own Anthropic API key (get one at
   console.anthropic.com). DEV calls the API directly from your phone; the key is stored only
   in the app's local settings and is sent solely to `api.anthropic.com`.
5. Back on the **DEV** tab, tap **Start listening**. Say "Hey DEV" and wait for it to respond
   "Yes?", then give a command, e.g. "Hey DEV, open Chrome."

## What's genuinely implemented

- **Voice loop**: continuous wake-word spotting via Android's built-in `SpeechRecognizer`
  (restarted in a loop checking partial results for "hey dev"), command capture, and spoken
...
- DEV never enters passwords, OTPs, or attempts to defeat CAPTCHA/biometric/2FA — it pauses
